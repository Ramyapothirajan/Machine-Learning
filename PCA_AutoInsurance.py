...
Num_pipeline = make_pipeline(MinMaxScaler(), pca)
Num_pipeline
...
# PCA weights
loaded_data['pca'].components_

# Take a closer look at the components
components = pd.DataFrame(loaded_data['pca'].components_, columns = numerical_features).T
components.columns = ['PCA_1', 'PCA_2', 'PCA_3', 'PCA_4', 'PCA_5', 'PCA_6', 'PCA_7']

components

# Principal Component Loadings
principal_components = pca.components_
principal_components_df = pd.DataFrame(principal_components, columns=df1_winsorized.columns)
plt.figure(figsize=(10, 6))
sns.heatmap(principal_components_df, annot=True, cmap='coolwarm')
...
plt.figure(figsize=(10, 6))
plt.bar(range(1, len(explained_variance) + 1), explained_variance_ratio, label='Explained Variance Ratio')
plt.plot(range(1, len(explained_variance) + 1), cumulative_variance_ratio, marker='o', label='Cumulative Explained Variance Ratio')
plt.xlabel('Principal Component')
plt.ylabel('Explained Variance Ratio')
plt.title('Scree Plot: Explained Variance Ratio and Cumulative Explained Variance')
plt.legend()
plt.show()
...
final_result = pd.concat([df1.State, df1['EmploymentStatus'], df1['Coverage'],df1['Renew Offer Type'], df1['Policy'],  data.iloc[:, 0:5]], axis = 1)
